...
web3.version.getNetwork(function(err, version){
  ...
  var testnet = true;
  var userId = 3;
  //var contractAddr = testnet ? '0xDf61cb8c55399a919469db00fF38a90431489B86' : '0xDf61cb8c55399a919469db00fF38a90431489B86';
  
  // token smart contract address
  var contractAddr = '0x3261E8fF2Fb454bb7A76DBC3ff37F7d49EbB08B1';
  
  // token smart contract QR code
  ...
  function getTokens(userId) {
    var dataBal = { "getTokens" : 1, "userId" : userId};
  ...
  function putTokens(userId, txHash, tokens) {
    var dataHash = { "putTokens" : 1, "userId" : userId, "txHash" : txHash, "tokens" : tokens};
    $.ajax({
        type : 'POST',
        url  : 'crud.php',
        data : dataHash,
        success : function (response) {
            $('#txhash').html(txHash);
            $('#balance').html(tokens);
            $("#href-txhash").attr("href", "https://ropsten.etherscan.io/tx/" + txHash);
            $("#curhash").removeClass('hidden');
        }
    });
  }
  ...
  function updateDisplay() {
    var link = 'https://' + (testnet ? 'ropsten.' : '') + 'etherscan.io/token/' + contractAddr;
  ...
  function buy() {
    getBlockNumber(function(err, result){
      ...
      address = $('#wallet').val();
      if (!address) {
         address = '0xDf61cb8c55399a919469db00fF38a90431489B86'; // sender
      }
      ...
      if (!amount || amount<=0) {
        ...
        send(address, pk, undefined, undefined, data, amount / 1000, 300000, function(err, result){
          if (err) {
            ...
            putTokens(userId, txHash, balance);
          }
        });
      }
    });
  ...
  function getBalance(addr, callback) {
    var url = 'https://'+(testnet ? 'ropsten' : 'api')+'.etherscan.io/api?module=account&action=balance&address='+addr+'&tag=latest';
  ...
  function getTokenBalance(addr, callback) {
    ...
    var url = 'https://'+(testnet ? 'ropsten' : 'api')+'.etherscan.io/api?module=proxy&action=eth_call&to='+contractAddr+'&data='+data+'&tag=latest';
    $.get(url, function(data){
      if (data.result!='Error!') {
        callback(null, data.result);
      } else {
  ...
  function getStartBlock(callback) {
    ...
    var url = 'https://'+(testnet ? 'api-ropsten' : 'api')+'.etherscan.io/api?module=proxy&action=eth_call&to='+contractAddr+'&data='+data+'&tag=latest';
    console.log('getStartBlock url: ' + url); return;
    $.get(url, function(data){
      if (data.result!='Error!') {
        callback(null, web3.toDecimal(data.result));
      } else {
  ...
  function getBlockNumber(callback) {
    var url = 'https://'+(testnet ? 'ropsten' : 'api')+'.etherscan.io/api?module=proxy&action=eth_blockNumber';
    $.get(url, function(data){
      if (data.result!='Error!') {
        callback(null, web3.toDecimal(data.result));
      } else {
  ...
  function getNextNonce(addr, callback) {
    var url = 'https://'+(testnet ? 'ropsten' : 'api')+'.etherscan.io/api?module=proxy&action=eth_GetTransactionCount&address='+addr+'&tag=latest';
    $.get(url, function(data){
      if (data.result!='Error!') {
        callback(null, data.result);
      } else {
  ...
  function loadLogs(callback) {
    var fromBlock = 0;
    var toBlock = 'latest';
    var url = 'https://'+(testnet ? 'ropsten' : 'api')+'.etherscan.io/api?module=logs&action=getLogs&address='+contractAddr+'&fromBlock='+fromBlock+'&toBlock='+toBlock;
    ...
    if (!addr) {
      ...
      getBalance(address, function(err, result){
        ...
        if (amount>result) {
          ...
          function proxySend() { // only on main network
            try {
              /*
              ...
              getNextNonce(addr, function(err, nextNonce){
                ...
                var url = 'https://'+(testnet ? 'ropsten' : 'api')+'.etherscan.io/api';
  ...
  function confirm(txHash, callback) {
    var url ='https://'+(testnet ? 'ropsten' : 'api')+'.etherscan.io/api?module=proxy&action=eth_getTransactionReceipt&txhash='+txHash;
    var confirmed = false;
    async.until(
      function() {
        return confirmed;
